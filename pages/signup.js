...
const Index = props => {
  ...
  useEffect(() => {
    if (data && data.tistory) {
      ...
      let isBlog = data.tistory.item.blogIds.length > 0;
      if (isBlog) {
        data.tistory.item.blogs.map(blog => {
          let form;
          form = {
          ...
          console.log("effect set form:", form);
        });
      }
    }
  }, [data]);
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (result) {
                signUp(id, pass, {
                  success: (sres, loaded) => {
                    console.log("signUp success", sres, formData);
                    absolve2(formData, sres.data && sres.data.userId)
                      .then(res => {
